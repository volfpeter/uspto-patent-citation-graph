...
class USPTOPatentCitationGraph(Graph):
    def __init__(
        self,
        database: Optional[GraphDatabaseInterface],
        *,
        log_neighbor_loading: bool = False,
    ) -> None:
        """
        Initialization.
    ...
    def create_default_database(reset: bool = False) -> GraphDatabaseInterface:
        ...
        engine = sqlalchemy.create_engine(
            "sqlite:///USPTOPatentCitationGraph.db", poolclass=StaticPool
        )
...
class USPTOPatentCitationNode(Node):
    def _load_neighbors_from_external_source(self) -> None:
        ...
        for patent, prefix in chain(
            zip(cited_patents, repeat("cited")), zip(citedby_patents, repeat("citedby"))
        ):
...
class USPTOPatentCitationNodeList(NodeList):
    def _create_node(
        self, index: int, name: str, external_id: Optional[str] = None
    ) -> Node:
        return USPTOPatentCitationNode(
            graph=self._graph, index=index, name=name, external_id=external_id
        )
